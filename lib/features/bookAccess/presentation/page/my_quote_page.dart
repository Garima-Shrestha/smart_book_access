...
class _MyQuotesPageState extends ConsumerState<MyQuotesPage> {
  ...
  Future<void> _fetchAccesses(int page, {required bool append}) async {
    if (!append) {
      await _loadQuotesFromCache(append: false);
      setState(() {
        _loadingMore = false;
        _hasMore = true;
        _page = 1;
    ...
    try {
      ...
      if (token == null || token.isEmpty) {
        if (!mounted) return;
        await _loadQuotesFromCache(append: append);
        return;
      }
  ...
  Future<void> _loadQuotesFromCache({required bool append}) async {
    try {
      ...
      for (final libItem in cachedLibrary) {
        ...
        rows.add(_BookAccessRow(
          id: bookAccess.id ?? bookAccess.bookId,
          book: _BookMini(
            id: libItem.bookId,
            title: bookHive?.title ?? libItem.title,
            author: bookHive?.author ?? libItem.author,
            coverImageUrl: bookHive?.coverImageUrl ?? libItem.coverImageUrl,
          ),
          isActive: !libItem.isExpired && !libItem.isInactive,
          quotes: quotes,
        ));
      }
